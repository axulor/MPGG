...
在多智能体系统（MAS）中促进合作的涌现是一个核心挑战。现实世界中的许多交互发生在连续空间中，个体的移动性深刻影响其交互模式和社会结果。本项目旨在探索在一个结合了**连续空间运动**与**公共物品博弈（PGG）** 的多智能体框架中，是否可以通过**学习**智能体的移动策略来有效促进合作行为的演化。我们提出一个模型，其中智能体在二维周期性空间中移动，根据其邻居进行 PGG 交互，并基于 Fermi 规则模仿邻居的博弈策略。关键在于，智能体的移动策略本身是可学习的，我们利用基于图神经网络（GNN）的多智能体强化学习（MARL）算法（MAPPO 和 IPPO）来优化移动策略，以最大化系统总收益或平均合作水平。我们期望通过系统的模拟实验，识别出能够促进合作的有效移动模式，分析其特征，并量化其对合作演化和社会福利的影响，为理解和设计能够自发产生合作的移动 MAS 提供见解。
...
*   **自身状态:**
    ...
    *   （可选）自身归一化位置 $\mathbf{x}_i(t)/L$。
    *   （可选）自身上一轮总收益 $\mathcal{R}_i(t-1)$。
    *   （可选）自身当前博弈策略 $a_i^g(t)$。
*   **邻居信息 (感知/交互半径 $r$ 内):** 对于每个邻居 $j \in \mathcal{N}_i(t) = \{k | \|\mathbf{x}_k(t) - \mathbf{x}_i(t)\| \le r, k \ne i\}$：
    ...
    *   邻居当前博弈策略 $a_j^g(t)$。
*   **实体类型ID:** 一个代表“智能体”的常量 ID（例如 0），用于 GNN 输入格式兼容。

*注：确切的观测内容需要根据实验调整，以提供足够信息给 GNN 和移动策略。*
...
*   **连续动作:** 可以是二维向量 $[\Delta v_x, \Delta v_y]$，表示速度方向的变化量或直接设置归一化的速度方向，范围通常在 [-1, 1]。
    *当前代码 (`marl_env.py`) 实现的是离散动作方式，直接设置目标速度方向。*
...
*   **对于 IPPO (Decentralized):** 使用个体奖励信号 $R_i(t) = \mathcal{R}_i(t)$。
...
2.  **策略特征:** 学习到的“最优”移动策略呈现出什么特征？是倾向于聚集（向所有邻居靠拢）、分离（远离所有邻居）、追随合作者、逃离背叛者，还是更复杂的组合？
...
*   为设计具有适应性移动能力、能够自组织形成合作的智能体群提供理论和算法基础。

------

## 6. 实施细节

*   **模拟器:** 基于 Python 和 NumPy/SciPy 实现的自定义多智能体模拟环境（即我们正在使用的代码库）。
*   **MARL 算法:** 基于 PyTorch 和 PyTorch Geometric 实现 MAPPO 和 IPPO，集成 GNN 模块（如 `graph_actor_critic.py`, `gnn.py` 等）。
*   **实验管理:** 使用工具（如 WandB, MLflow）记录实验参数、指标和结果。
